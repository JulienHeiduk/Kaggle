...
train=pd.read_csv('C:/Users/Ju/Desktop/Kaggle/Shelter/train.csv',delimiter=';')
test=pd.read_csv('C:/Users/Ju/Desktop/Kaggle/Shelter/test.csv',delimiter=';')
pd.options.mode.chained_assignment = None

#train = pd.DataFrame(train[train['AnimalType'] == 'Dog'])
#print(train)
...
def clean(tr,ts):
    ...
    tr['AgeuponOutcome_cat']=0
    ts['AgeuponOutcome_cat']=0
...
for i in range(0,l3.shape[0]):
    if l3['Groups'][i]=='FSS':
        l3['Groups_cat'][i]=1
    if l3['Groups'][i]=='Herding':
        l3['Groups_cat'][i]=2
    if l3['Groups'][i]=='Hound':
        l3['Groups_cat'][i]=3
    if l3['Groups'][i]=='MSC':
        l3['Groups_cat'][i]=4
    if l3['Groups'][i]=='Non_sporting':
        l3['Groups_cat'][i]=5
    if l3['Groups'][i]=='Sporting':
        l3['Groups_cat'][i]=6
    if l3['Groups'][i]=='Terrier':
        l3['Groups_cat'][i]=7
    if l3['Groups'][i]=='Toy':
        l3['Groups_cat'][i]=8
    if l3['Groups'][i]=='Working':
        l3['Groups_cat'][i]=9          
...
#Type3
l1111=['Bulldog','Shih Tzu','Cavalier King Charles Spaniel','Pug','Bichon Frise','French Bulldog','Greyhound','Havanese','Maltese','Chinese Crested','Labrador Retriever','Bulldog','Golden Retriever','Beagle','Pug','Irish Setter','Brussels Griffon','Newfoundland','French Bulldog','Collie','Bullmastiff','Doberman Pinsch','Giant Schnauzer','Akita','German Shepherd Dog','Staffordshire Bull Terrier','Rottweiler','Puli','Rhodesian Ridgeback','Chinese Crested','Peruvian Inca Orchid','American Hairless Terrier','Xoloitzcuintli','Bedlington Terrier','Bichon Frise','Chinese Crested','Coton de Tulear','Irish Water Spaniel','Kerry Blue Terrier','Maltese','Poodle','Portuguese Water Dog','Standard Schnauzer','Giant Schnauzer','Soft Coated Wheaten Terrier','Xoloitzcuintli','American Hairless Terrier','Lagotto Romagnolo','Afghan Hound','Peruvian Inca Orchid','Bulldog','Beagle','Newfoundland','Golden Retriever','Labrador Retriever','Soft Coated Wheaten Terrier','Weimaraner','Bull Terrier','Boxer','Border Collie','Poodle','German Shepherd Dog','Golden Retriever','Doberman Pinsch','Shetland Sheepdog','Labrador Retriever','Papillon','Bloodhound','Rottweiler']
l2222=['Appartments','Appartments','Appartments','Appartments','Appartments','Appartments','Appartments','Appartments','Appartments','Appartments','Family','Family','Family','Family','Family','Family','Family','Family','Family','Family','Guard','Guard','Guard','Guard','Guard','Guard','Guard','Guard','Guard','Hairless','Hairless','Hairless','Hairless','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Hypoallergenic','Kids','Kids','Kids','Kids','Kids','Kids','Kids','Kids','Kids','Smart','Smart','Smart','Smart','Smart','Smart','Smart','Smart','Smart','Smart']

l14=pd.DataFrame(l1111)
l14['Breed11']=pd.DataFrame(l1111)
l14['Type3']=pd.DataFrame(l2222)
l14.drop(0,axis=1,inplace=True)
l14['Breed11']=l14['Breed11'].str.upper()

l144=pd.DataFrame(l1111)
l144['Breed2']=pd.DataFrame(l1111)
l144['Type3_2']=pd.DataFrame(l2222)
l144.drop(0,axis=1,inplace=True)
l144['Breed2']=l144['Breed2'].str.upper()

train=pd.merge(train, l13, on = 'Breed11',how='left')
train=pd.merge(train, l14, on = 'Breed11',how='left')
train=pd.merge(train, l144, on = 'Breed2',how='left')
...
#Type4
#Doublons a refaire d'apres le fichier word....
...
train = pd.get_dummies(train, columns=['Groups_cat','Type2','Type3','Type3_2','DG','Type4','dayofweek','Holidays','Season','am_pm','Unicolor','month','day',
